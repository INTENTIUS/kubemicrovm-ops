...
**Re-running is safe.** A cluster and a floci container that are already up are reused rather than replaced, so `./go` twice does not destroy what the first one built — which matters because behold wires its substrate **Bring up** button to this same script, and a click on a healthy estate used to tear it down. `RECREATE_CLUSTER=1` rebuilds both from nothing, which is what you want after changing m80's flags, since those live in the deployment's args.

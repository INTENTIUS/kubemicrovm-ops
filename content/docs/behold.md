...
A standalone MicroVM emulator changes this, and one is now in design (working title squib, mudflaps-style, currently a local docs-only repo). Both the operator and the `microvm` CLI reach AWS through the SDK, which takes an endpoint override, so squib plus k3d gives the kit a full local loop and behold its apply demo. The CloudFormation side follows separately in floci. See the [Roadmap]({{< relref "roadmap" >}})'s resolved section.
